...
const AdAccountReconciliationTable = ({ data, loading }) => {
    ...
    const totalSpendVariance = data.rows.reduce((sum, r) => sum + (r.spendVariance || 0), 0);
                    ...
                    <table className="w-full text-[11px]">
                        <thead className="bg-gray-50/80 sticky top-0 z-10 border-b border-gray-100 backdrop-blur-sm">
                            <tr>
                                ...
                                <th className="px-3 py-2 text-right border-r border-gray-100">
                                    <div className="text-[10px] font-bold text-gray-600 uppercase tracking-wider">Chênh lệch chi tiêu</div>
                                    <div className="text-[8px] text-gray-400 font-normal lowercase">(Tiêu ghi nhận - Tiêu thực)</div>
                                </th>
                                <th className="px-3 py-2 text-right border-r border-gray-100">
                        ...
                        <tbody className="bg-white divide-y divide-gray-100">
                            {data.rows.map((row, index) => (
                                <tr key={row.adAccountId || index} className="hover:bg-blue-50/30 transition-all group">
                                    ...
                                    <td className="px-3 py-2 text-right border-r border-gray-50">
                                        <div className={`font-bold ${Math.abs(row.spendVariance) > 1000 ? 'text-orange-500' : 'text-gray-700'}`}>
                                            {formatCurrency(row.spendVariance)}
                                        </div>
                                    </td>
                                    <td className="px-3 py-2 text-right border-r border-gray-50">
                        ...
                        <tfoot className="bg-gray-50/80 sticky bottom-0 z-10 border-t border-gray-200 backdrop-blur-sm">
                            <tr className="font-bold text-gray-800">
                                ...
                                <td className="px-3 py-2 text-right border-r border-gray-100">
                                    <div className="flex flex-col items-end">
                                        <span className="text-[8px] text-gray-400 uppercase tracking-wider">Tổng chênh tiêu</span>
                                        <span className="text-orange-500">{formatCurrency(totalSpendVariance)}</span>
                                    </div>
                                </td>
                                <td className="px-3 py-2 text-right border-r border-gray-100">
                                    <div className="flex flex-col items-end">
